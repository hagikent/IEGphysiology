...
%%
COI3=[125 268 118 735 103];
for kk=1:length(COI3)
    ii=COI3(kk);
...
%%
%COIfig=[9, 363, 125, 268, 735, 103];
%figure,
%for ii=1:6
%subplot(6,2,(ii-1)*2+1),bar(DataUnitsAll.autocorr{COIfig(ii)},1,'FaceColor',[0.5 0.5 0.5])
%subplot(6,2,ii*2),plot(DataUnitsAll.kilosort_wf{COIfig(ii)})
%end
%% tone / opto responses
...
for ii=1:length(CS_posi)
    %plot(mean(PSTH_CS2All{CS_posi(ii)}),Color=[0.9 0.5 0.5 0.5]),hold on
    %PSTH_CS2All_CS_posi = [PSTH_CS2All_CS_posi; mean(PSTH_CS2All{CS_posi(ii)})];
    %plot(mean(PSTH_CS2Allz{CS_posi(ii)}),Color=[0.9 0.5 0.5 0.5]),hold on
    PSTH_CS2All_CS_posi = [PSTH_CS2All_CS_posi; mean(PSTH_CS2Allz{CS_posi(ii)})]; %z=scored
end

for ii=1:length(CS_nega)
    %plot(mean(PSTH_CS2All{CS_nega(ii)}),Color=[0.5 0.5 0.9 0.5]),hold on
    %PSTH_CS2All_CS_nega = [PSTH_CS2All_CS_nega; mean(PSTH_CS2All{CS_nega(ii)})];
    %plot(mean(PSTH_CS2Allz{CS_nega(ii)}),Color=[0.5 0.5 0.9 0.5]),hold on
...
for ii=1:length(Opto_posi)
    %plot(mean(PSTH_opto20Hz2All{Opto_posi(ii)}),Color=[0.9 0.5 0.5 0.5]),hold on
    %PSTH_opto20Hz2All_Opto_posi = [PSTH_opto20Hz2All_Opto_posi; mean(PSTH_opto20Hz2All{Opto_posi(ii)})];
    %plot(mean(PSTH_opto20Hz2Allz{Opto_posi(ii)}),Color=[0.9 0.5 0.5 0.5]),hold on
...
for ii=1:length(Opto_nega)
    %plot(mean(PSTH_opto20Hz2All{Opto_nega(ii)}),Color=[0.5 0.5 0.9 0.5]),hold on
    %PSTH_opto20Hz2All_Opto_nega = [PSTH_opto20Hz2All_Opto_nega; mean(PSTH_opto20Hz2All{Opto_nega(ii)})];
    %plot(mean(PSTH_opto20Hz2Allz{Opto_nega(ii)}),Color=[0.5 0.5 0.9 0.5]),hold on
...
plot(linspace(0,1,1471),aa,'k'),hold on
[aa,bb]=sort(CS_sig/max(CS_sig));
plot(linspace(0,1,1471),aa,'r'),hold on
[aa,bb]=sort(Opto_sig/max(Opto_sig));
plot(linspace(0,1,1471),aa,'b'),hold on
grid on
...
contrbution_nc = ones(1,12)-contrbution_tag-contrbution_exc-contrbution_inh;
...
subplot(2,4,5)
plot([1 2],[zeros(1,12); (d_CS_Opto-d_CS_base)./d_CS_base],'-',Color=[0.6 0.6 0.6]),hold on
...
plot([0 3], [0 0], '--')
xlim([0.5 2.5])
% (d_CS_Opto-d_CS_base)./d_CS_base] = [-0.6013    0.0089    0.2447
% -0.8303   -0.7378   -0.7087   -0.2323   -0.7246   -0.9418   -0.7882   -0.2656    0.0882]
...
Agg=[contrbution_tag_pcell contrbution_exc_pcell contrbution_inh_pcell contrbution_nc_pcell];
group={'A', 'A', 'A', 'A', 'A', 'A', 'A', 'A', 'A', 'A', 'A', 'A',...
    'B', 'B', 'B','B', 'B', 'B','B', 'B', 'B','B', 'B', 'B',...
    'C', 'C', 'C','C', 'C', 'C','C', 'C', 'C','C', 'C', 'C',...
    'D', 'D', 'D','D', 'D', 'D','D', 'D', 'D','D', 'D', 'D'};
[p,tbl,stats] = anova1(Agg,group);
%p_Contribution = dunnett(stats, [], [1])

% Freezing Anal

FreezingFCAll=[];
Ephys_FC{1}='\\tungsten-nas.fmi.ch\tungsten\scratch\gluthi\hagikent\IEGpaper\behavior\IEG_EphysBehavAnal\eng10_s01_fc\';
Ephys_FC{2}='\\tungsten-nas.fmi.ch\tungsten\scratch\gluthi\hagikent\IEGpaper\behavior\IEG_EphysBehavAnal\eng11_s01_fc\';
Ephys_FC{3}='\\tungsten-nas.fmi.ch\tungsten\scratch\gluthi\hagikent\IEGpaper\behavior\IEG_EphysBehavAnal\eng12_s01_fc\';
Ephys_FC{4}='\\tungsten-nas.fmi.ch\tungsten\scratch\gluthi\hagikent\IEGpaper\behavior\IEG_EphysBehavAnal\eng13_s01_fc\';
Ephys_FC{5}='\\tungsten-nas.fmi.ch\tungsten\scratch\gluthi\hagikent\IEGpaper\behavior\IEG_EphysBehavAnal\eng14_s01_fc\';
Ephys_FC{6}='\\tungsten-nas.fmi.ch\tungsten\scratch\gluthi\hagikent\IEGpaper\behavior\IEG_EphysBehavAnal\eng15_s01_fc\';
Ephys_FC{7}='\\tungsten-nas.fmi.ch\tungsten\scratch\gluthi\hagikent\IEGpaper\behavior\IEG_EphysBehavAnal\eng16_s01_fc\';
Ephys_FC{8}='\\tungsten-nas.fmi.ch\tungsten\scratch\gluthi\hagikent\IEGpaper\behavior\IEG_EphysBehavAnal\eng17_s01_fc\';
Ephys_FC{9}='\\tungsten-nas.fmi.ch\tungsten\scratch\gluthi\hagikent\IEGpaper\behavior\IEG_EphysBehavAnal\eng19_s01_fc\';
Ephys_FC{10}='\\tungsten-nas.fmi.ch\tungsten\scratch\gluthi\hagikent\IEGpaper\behavior\IEG_EphysBehavAnal\eng20_s01_fc\';
Ephys_FC{11}='\\tungsten-nas.fmi.ch\tungsten\scratch\gluthi\hagikent\IEGpaper\behavior\IEG_EphysBehavAnal\eng21_s01_fc\';
Ephys_FC{12}='\\tungsten-nas.fmi.ch\tungsten\scratch\gluthi\hagikent\IEGpaper\behavior\IEG_EphysBehavAnal\eng22_s01_fc\';
...
FreezingAll=[];
Ephys_FC{1}='\\tungsten-nas.fmi.ch\tungsten\scratch\gluthi\hagikent\IEGpaper\behavior\IEG_EphysBehavAnal\eng10_s02_retrieval\';
Ephys_FC{2}='\\tungsten-nas.fmi.ch\tungsten\scratch\gluthi\hagikent\IEGpaper\behavior\IEG_EphysBehavAnal\eng11_s02_retrieval\';
Ephys_FC{3}='\\tungsten-nas.fmi.ch\tungsten\scratch\gluthi\hagikent\IEGpaper\behavior\IEG_EphysBehavAnal\eng12_s02_retrieval\';
Ephys_FC{4}='\\tungsten-nas.fmi.ch\tungsten\scratch\gluthi\hagikent\IEGpaper\behavior\IEG_EphysBehavAnal\eng13_s02_retrieval\';
Ephys_FC{5}='\\tungsten-nas.fmi.ch\tungsten\scratch\gluthi\hagikent\IEGpaper\behavior\IEG_EphysBehavAnal\eng14_s02_retrieval\';
Ephys_FC{6}='\\tungsten-nas.fmi.ch\tungsten\scratch\gluthi\hagikent\IEGpaper\behavior\IEG_EphysBehavAnal\eng15_s02_retrieval\';
Ephys_FC{7}='\\tungsten-nas.fmi.ch\tungsten\scratch\gluthi\hagikent\IEGpaper\behavior\IEG_EphysBehavAnal\eng16_s02_retrieval\';
Ephys_FC{8}='\\tungsten-nas.fmi.ch\tungsten\scratch\gluthi\hagikent\IEGpaper\behavior\IEG_EphysBehavAnal\eng17_s02_retrieval\';
Ephys_FC{9}='\\tungsten-nas.fmi.ch\tungsten\scratch\gluthi\hagikent\IEGpaper\behavior\IEG_EphysBehavAnal\eng19_s02_retrieval\';
Ephys_FC{10}='\\tungsten-nas.fmi.ch\tungsten\scratch\gluthi\hagikent\IEGpaper\behavior\IEG_EphysBehavAnal\eng20_s02_retrieval\';
Ephys_FC{11}='\\tungsten-nas.fmi.ch\tungsten\scratch\gluthi\hagikent\IEGpaper\behavior\IEG_EphysBehavAnal\eng21_s02_retrieval\';
Ephys_FC{12}='\\tungsten-nas.fmi.ch\tungsten\scratch\gluthi\hagikent\IEGpaper\behavior\IEG_EphysBehavAnal\eng22_s02_retrieval\';
...
%%
figure,
scatter(dFreezingFC_late, Freezing, 50, [0.5 0.5 0.5],'filled')

scatter(dFreezingFC_late,Freezing,50,[0.5 0.5 0.5],'filled'),hold on

[p, R2, p_val,y_fit] = linear_fitKH(dFreezingFC_late,Freezing);
R = corrcoef(XX,YY);

xlabel('dFreezing in FC')
ylabel('Retr.Freezing')
%title(['p = :' num2str(p_val) '; R = :' num2str(R(1,2))])
xlim([0 1])
ylim([0 1])

%% Fos Arc Mahal Summary

Mahal_Fos = [-0.6013    0.0089    0.2447  -0.8303   -0.7378   -0.7087   -0.2323   -0.7246   -0.9418   -0.7882   -0.2656    0.0882];
Mahal_Arc = [-0.6821    0.8885   -0.3896    1.1500    0.5750   -0.2504    0.6597   -0.2637   -0.1717   -0.4667];

figure, 
set(gcf,'position',[100,100,1200,400])
subplot(1,6,1),
errorbar(1,mean(Mahal_Fos),std(Mahal_Fos)/sqrt(length(Mahal_Fos)),'o'),hold on
scatter(1,Mahal_Fos,'.k')
errorbar(2,mean(Mahal_Arc),std(Mahal_Fos)/sqrt(length(Mahal_Arc)),'o')
scatter(2,Mahal_Arc,'.k')
plot([0.5 2.5],[0 0],'--')
xlim([0.5 2.5])

p_FosMD=ranksum(Mahal_Fos, zeros(1,12))
p_ArcMD=ranksum(Mahal_Arc, zeros(1,10))

subplot(1,6,3:4),
%XX=(d_CS_Opto-d_CS_base)./d_CS_base;
XX_Fos=Mahal_Fos;
%YY=dFreezingFC_late;
YY_Fos=[0.3667    0.1591    0.1158    0.6761    0.2968    0.2827    0.3058    0.3522    0.4217    0.3182    0.3201    0.3307]';
scatter(XX_Fos,YY_Fos,50,[0.5 0.5 0.5],'filled'),hold on

[p, R2, p_val,y_fit] = linear_fitKH(XX_Fos',YY_Fos);
R = corrcoef(XX_Fos',YY_Fos);
plot(XX_Fos', y_fit, 'b-',LineWidth=2)
ylim([0 0.8])
ylabel('FC freezing 3:5 - Base')
xlabel('Relative PVD change')

mdl = fitlm(XX_Fos', YY_Fos, 'linear');
plot(mdl)
...
patch([xData, fliplr(xData)], [yLower, fliplr(yUpper)], 'b', 'FaceAlpha', 0.1);
title(['p = :' num2str(p_val) '; R = :' num2str(R(1,2))])
...
XX_Arc=Mahal_Arc;
%YY=dFreezingFC_late;
YY_Arc=[-0.0434    0.0818    0.2399    0.0695    0.4696    0.2727    0.4773   -0.2810   -0.0762    0.2104]';
scatter(XX_Arc,YY_Arc,50,[0.5 0.5 0.5],'filled'),hold on

[p, R2, p_val,y_fit] = linear_fitKH(XX_Arc,YY_Arc');
R = corrcoef(XX_Arc,YY_Arc');
plot(XX_Arc, y_fit, 'b-',LineWidth=2)
ylim([-0.3 0.8])

%ylabel('FC freezing 3:5 - Base')
xlabel('Relative PVD change')

mdl = fitlm(XX_Arc', YY_Arc, 'linear');
plot(mdl)
...
yUpper = h(1).YData;
yLower = h(2).YData;
patch([xData, fliplr(xData)], [yLower, fliplr(yUpper)], 'r', 'FaceAlpha', 0.1);
title(['p = :' num2str(p_val) '; R = :' num2str(R(1,2))])
title(['p = :' num2str(p_val) '; R = :' num2str(R(1,2))])
%%
figure, 
...
subplot(1,6,3:4),
XX_Fos=Mahal_Fos;
YY_Fos=[0.6947    0.4085    0.5665    0.6570    0.5388    0.4182    0.6278    0.6907    0.6913    0.5617    0.4542    0.4794]';
scatter(XX_Fos,YY_Fos,50,[0.5 0.5 0.5],'filled'),hold on
...
subplot(1,6,5:6),
XX_Arc=Mahal_Arc;
YY_Arc=[0.4400    0.6733    0.2273    0.7076    0.2100    0.4188    0.5855    0.5227    0.5897    0.3666]';
scatter(XX_Arc,YY_Arc,50,[0.5 0.5 0.5],'filled'),hold on
...
%% Fos Arc Opto Responses

baselineFRtag_Fos=[0.4833    1.1700    2.6000    7.9733    4.1667    0.0433    1.3733    0.9433    0.4533    1.2800    7.4467    4.6333    1.2133    0.7133    0.4700    0.0100...
    0.0333    0.0533    0.0267    0.0567    0.4000    0.2167    5.7833    0.0333];

OptoFRtag_Fos=[0.4567    1.1233    2.3300    8.1567    4.1300    0.1633    1.3967    1.7067    2.0400    1.3900    7.0433    4.9333    6.2767    4.3467    0.5933    3.2600...
    10.7800    3.3667    7.8900    7.5133    0.4400    3.6200    5.0000    9.9433];


baselineFRtag_Arc=[0.4267    0.6800    0.3133    0.6933    1.2900    2.0633    0.4100    0.8633    0.3900    0.0533    0.7267    0.8867    2.0433    0.6133    0.1667    0.4600...
0.2100    0.6233    0.0800    0.1600    1.2767    1.1967    0.1200    3.6233    0.6433    0.7733    0.2733    5.0367    0.4033    1.7167    0.3567    0.0933...
0.4400    1.7467    0.9233];

OptoFRtag_Arc=[1.0200    1.1300    1.7133    1.9133    6.2900    3.4633    4.4167    2.1133    1.2133    2.1400    3.9300    2.3367   15.2067    0.9767    3.1867    3.2867...
    4.6300    1.5633    8.5100   10.3033   11.8667    8.8633    3.8933    8.8700    2.1133    4.5733    5.2500    6.0900    6.9967    1.4700    2.5600    0.1233...
    3.1133    2.6367    0.0500];

p=ranksum(OptoFRtag_Fos,OptoFRtag_Arc)

figure,
errorbar(0.8,mean(baselineFRtag_Fos),std(baselineFRtag_Fos)/sqrt(length(baselineFRtag_Fos)),'ob'),hold on
errorbar(1.8,mean(OptoFRtag_Fos),std(OptoFRtag_Fos)/sqrt(length(OptoFRtag_Fos)),'ob')
errorbar(1.2,mean(baselineFRtag_Arc),std(baselineFRtag_Arc)/sqrt(length(baselineFRtag_Arc)),'or'),hold on
errorbar(2.2,mean(OptoFRtag_Arc),std(OptoFRtag_Arc)/sqrt(length(OptoFRtag_Arc)),'or')
xlim([0.5 2.5])
ylim([0 5])
title(['OptoResp, p=' num2str(p)])

display(['Fos OptoFR: ' num2str(mean(OptoFRtag_Fos)) '+-' num2str(std(OptoFRtag_Fos)/sqrt(length(OptoFRtag_Fos)))])
display(['Arc OptoFR: ' num2str(mean(OptoFRtag_Arc)) '+-' num2str(std(OptoFRtag_Arc)/sqrt(length(OptoFRtag_Arc)))])
display(['ranksum p = ' num2str(p)])
...
plot(mean(Opto_CDCS,2),color = [0.2 0.2 0.7],LineWidth=2)
ylim([-1,2])
